...
import {
  useCreateSalesReturnItem,
  useSalesReturnItems,
  useDeleteSalesReturnItem,
} from "@/hooks/use-sales-return-items"
...
export function SalesReturnItemsSection({
  salesReturnId,
  invoiceId,
  editable,
}: {
  ...
  const { data: returnLines } = useSalesReturnItems(salesReturnId)
  const createReturnLine = useCreateSalesReturnItem()
  const deleteReturnLine = useDeleteSalesReturnItem(salesReturnId)

  const [toDelete, setToDelete] = useState<SalesReturnItem | null>(null)
  const [pendingQty, setPendingQty] = useState<Record<string, number>>({})

  const returnedByInvoiceLine = new Map<string, number>()
  returnLines?.forEach((line) => {
    if (!line.invoice_item_id) return
    returnedByInvoiceLine.set(
      line.invoice_item_id,
      (returnedByInvoiceLine.get(line.invoice_item_id) ?? 0) + line.quantity
    )
  })
  ...
  const availableLines =
    invoiceLines?.filter(
      (line) => (returnedByInvoiceLine.get(line.id) ?? 0) < line.quantity
    ) ?? []
              ...
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">
                    {tInvoices("quantityLabel")}
                  </TableHead>
                  <TableHead className="text-right">
                    {tInvoices("unitPriceLabel")}
                  </TableHead>
                  <TableHead className="text-right">
                    {tInvoices("lineTotalLabel")}
                  </TableHead>
            ...
            <div className="flex flex-col gap-2.5">
              {availableLines.map((line) => {
                ...
                return (
                  <div
                    key={line.id}
                    className="flex flex-col items-stretch gap-2 rounded-lg border p-3 sm:flex-row sm:items-center sm:gap-3"
                  >
                    ...
                    <div className="flex items-center justify-end gap-2">
                      <Input
                        ...
                        className="h-9 w-24 text-xs sm:text-sm"
                        onChange={(e) =>
                          setPendingQty((prev) => ({
                            ...prev,
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => addReturnLine(line.id)}
                        disabled={createReturnLine.isPending}
                        className="shrink-0"
                      >
                        <PlusIcon />
                        {t("addReturnLine")}
