...
export async function GET() {
  ...
  try {
    const [invoicesResult, billsResult, invoiceItemsResult, paymentsResult] =
      await Promise.all([
        supabase
          .schema("billing")
          .from("invoices")
    ...
    for (const rawLine of invoiceItems) {
      const it = (
        Array.isArray(rawLine.item) ? rawLine.item[0] : rawLine.item
      ) as { id: string; name: string; sku: string | null } | undefined | null
      if (!it) continue
    ...
    const customerMap: Record<
      string,
      { name: string; totalInvoiced: number; invoiceCount: number }
    > = {}
    for (const rawInv of invoices) {
      const cust = (
        Array.isArray(rawInv.customer) ? rawInv.customer[0] : rawInv.customer
