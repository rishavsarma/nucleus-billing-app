...
async function getListVersion(orgId: string): Promise<number> {
  try {
    const v = await redis.get(`offer-list-version:${orgId}`)
...
async function bumpListVersion(orgId: string): Promise<void> {
  try {
    await redis.incr(`offer-list-version:${orgId}`)
  } catch {
    // swallow
  }
}
...
export async function GET(request: Request) {
  ...
  if (id) {
    ...
    let recordQuery = supabase.schema("billing").from("offers").select("*").eq("id", id)
    ...
    if (!auth.isSuperadmin) {
      await cacheSet(offerCacheKey(auth.orgId!, id), JSON.stringify(data), OFFER_CACHE_TTL_SECONDS)
    }
  ...
  if (!auth.isSuperadmin) {
    ...
    const listKey = offerListCacheKey(auth.orgId!, version, search, page, pageSize)
    const cached = await cacheGet(listKey)
    if (cached) return NextResponse.json(JSON.parse(cached))

    let query = supabase.schema("billing").from("offers").select("*", { count: "exact" })
...
export async function PUT(request: Request) {
  ...
  const body = await request.json()
  const supabase = auth.supabase
  let query = supabase.schema("billing").from("offers").update(body).eq("id", id)
