...
async function getListVersion(orgId: string): Promise<number> {
  try {
    const v = await redis.get(`tax-rate-list-version:${orgId}`)
...
async function bumpListVersion(orgId: string): Promise<void> {
  try {
    await redis.incr(`tax-rate-list-version:${orgId}`)
  } catch {
    // swallow
  }
}

function taxListCacheKey(orgId: string, version: number, search: string, page: number, pageSize: number) {
...
export async function GET(request: Request) {
  ...
  if (id) {
    ...
    let recordQuery = supabase.schema("billing").from("tax_rates").select("*").eq("id", id)
    ...
    if (!auth.isSuperadmin) {
      await cacheSet(taxCacheKey(auth.orgId!, id), JSON.stringify(data), TAX_CACHE_TTL_SECONDS)
    }
  ...
  if (!auth.isSuperadmin) {
    ...
    const listKey = taxListCacheKey(auth.orgId!, version, search, page, pageSize)
    const cached = await cacheGet(listKey)
    if (cached) return NextResponse.json(JSON.parse(cached))

    let query = supabase.schema("billing").from("tax_rates").select("*", { count: "exact" })
    ...
    if (error) {
      if (isRangeError(error)) {
        ...
        await cacheSet(listKey, JSON.stringify(emptyPayload), TAX_CACHE_TTL_SECONDS)
...
export async function PUT(request: Request) {
  ...
  const body = await request.json()
  const supabase = auth.supabase
  let query = supabase.schema("billing").from("tax_rates").update(body).eq("id", id)
