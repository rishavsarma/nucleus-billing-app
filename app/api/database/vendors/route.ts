...
export async function GET(request: Request) {
  ...
  if (!auth.isSuperadmin) {
    const version = await cacheGetListVersion("vendor", auth.orgId!)
    const listKey = vendorListCacheKey(
      auth.orgId!,
      version,
      search,
      page,
    ...
    if (error) {
      ...
      return dbError(error, "vendors:GET")
    }
...
export async function POST(request: Request) {
  ...
  const { data, error } = await supabase
    .schema("billing")
    .from("vendors")
    .insert({ ...pickAllowed("vendors", body), org_id: orgId })
    .select()
    .single()

  if (error) return dbError(error, "vendors:POST")
  void cacheBumpListVersion("vendor", orgId)
...
export async function PUT(request: Request) {
  ...
  let query = supabase
    .schema("billing")
    .from("vendors")
    .update(pickAllowed("vendors", body))
    .eq("id", id)
  if (!auth.isSuperadmin) query = query.eq("org_id", auth.orgId)
  const { data, error } = await query.select().maybeSingle()

  if (error) return dbError(error, "vendors:PUT")
  if (!data) return notFound()

  await cacheDel(vendorCacheKey(data.org_id, id))
  void cacheBumpListVersion("vendor", data.org_id)
  return NextResponse.json(data)
}
...
export async function DELETE(request: Request) {
  ...
  if (error) return dbError(error, "vendors:DELETE")
  if (data) {
    await cacheDel(vendorCacheKey(data.org_id, id))
    void cacheBumpListVersion("vendor", data.org_id)
  }
