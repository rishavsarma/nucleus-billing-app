...
async function getListVersion(orgId: string): Promise<number> {
  try {
    const v = await redis.get(`customer-list-version:${orgId}`)
...
async function bumpListVersion(orgId: string): Promise<void> {
  try {
    await redis.incr(`customer-list-version:${orgId}`)
  } catch {
    // swallow
  }
}
...
export async function GET(request: Request) {
  ...
  if (id) {
    ...
    let recordQuery = supabase.schema("billing").from("customers").select("*").eq("id", id)
    ...
    if (!auth.isSuperadmin) {
      await cacheSet(customerCacheKey(auth.orgId!, id), JSON.stringify(data), CUSTOMER_CACHE_TTL_SECONDS)
    }
  ...
  if (!auth.isSuperadmin) {
    ...
    const listKey = customerListCacheKey(auth.orgId!, version, search, page, pageSize)
    const cached = await cacheGet(listKey)
    if (cached) return NextResponse.json(JSON.parse(cached))

    let query = supabase.schema("billing").from("customers").select("*", { count: "exact" }).eq("org_id", auth.orgId)
    ...
    if (error) {
      if (isRangeError(error)) {
        ...
        await cacheSet(listKey, JSON.stringify(emptyPayload), CUSTOMER_CACHE_TTL_SECONDS)
  ...
  let query = supabase.schema("billing").from("customers").select("*", { count: "exact" })
  query = applyListParams(query, ["name", "email", "phone"], { search: search || undefined, page, pageSize })
  const { data, error, count } = await query
...
export async function PUT(request: Request) {
  ...
  const body = await request.json()
  const supabase = auth.supabase
  let query = supabase.schema("billing").from("customers").update(body).eq("id", id)
