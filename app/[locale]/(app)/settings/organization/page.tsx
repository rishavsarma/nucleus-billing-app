...
function OrganizationForm({ organization }: { organization: Organization }) {
  ...
  const lowStockAlertsEnabled = useWatch({
    control,
    name: "low_stock_alerts_enabled",
  })
  const financialYearStartMonth = useWatch({
    control,
  ...
  return (
    <div className="flex flex-col gap-6">
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
          ...
          <div className="p-4">
            <FieldGroup>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <Field>
                  <FieldLabel htmlFor="org-address">
                    {t("addressLabel")}
                  </FieldLabel>
          ...
          <div className="p-4">
            <FieldGroup>
              <div className="grid grid-cols-3 gap-4">
                ...
                <Field>
                  <FieldLabel htmlFor="org-state-code">
                    {t("stateCodeLabel")}
                  </FieldLabel>
              ...
              <Field orientation="horizontal">
                <div>
                  <FieldLabel htmlFor="org-gst-registered">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) =>
                    setValue("gst_registered", checked)
                  }
                />
              </Field>
            </FieldGroup>
          ...
          <div className="p-4">
            <FieldGroup>
              <div className="grid grid-cols-4 gap-4">
                <Field>
                  <FieldLabel htmlFor="org-invoice-prefix">
                ...
                <Field>
                  <FieldLabel htmlFor="org-bill-prefix">
                    {t("billPrefixLabel")}
                  </FieldLabel>
                  <Input id="org-bill-prefix" {...register("bill_prefix")} />
                </Field>
                <Field>
                  <FieldLabel htmlFor="org-credit-note-prefix">
                    {t("creditNotePrefixLabel")}
                  </FieldLabel>
                  <Input
                    id="org-credit-note-prefix"
                    {...register("credit_note_prefix")}
                  />
                </Field>
                <Field>
                  <FieldLabel htmlFor="org-debit-note-prefix">
                    {t("debitNotePrefixLabel")}
                  </FieldLabel>
                  <Input
                    id="org-debit-note-prefix"
                    {...register("debit_note_prefix")}
                  />
                </Field>
              </div>
            </FieldGroup>
        ...
        <div className="rounded-xl bg-card ring-1 ring-foreground/10">
          ...
          <div className="p-4">
            <FieldGroup>
              <Field>
                <FieldLabel htmlFor="org-watermark">
                  {t("watermarkLabel")}
                </FieldLabel>
              ...
              <Field>
                <FieldLabel htmlFor="org-logo-url">
                  {t("logoUrlLabel")}
                </FieldLabel>
                <Input id="org-logo-url" {...register("pdf_logo_url")} />
              </Field>
              <Field>
                <FieldLabel htmlFor="org-footer-notes">
                  {t("footerNotesLabel")}
                </FieldLabel>
                <Input
                  id="org-footer-notes"
                  {...register("pdf_footer_notes")}
                />
              </Field>
            </FieldGroup>
          </div>
        ...
        <div className="rounded-xl bg-card ring-1 ring-foreground/10">
          ...
          <div className="p-4">
            <FieldGroup>
              <Field>
                <FieldLabel htmlFor="org-fy-start">
                  {t("financialYearLabel")}
                </FieldLabel>
                <Select
                  value={String(financialYearStartMonth)}
                  onValueChange={(value) =>
                    setValue("financial_year_start_month", Number(value))
                  }
                >
              ...
              <Field orientation="horizontal">
                <div>
                  <FieldLabel htmlFor="org-low-stock-alerts">
                    {t("lowStockAlertsLabel")}
                  </FieldLabel>
                  <p className="text-xs text-muted-foreground">
                    {t("lowStockAlertsHint")}
                  </p>
                </div>
                <Switch
                  ...
                  onCheckedChange={(checked) =>
                    setValue("low_stock_alerts_enabled", checked)
                  }
                />
              </Field>
            </FieldGroup>
      ...
      <OrganizationSignatureCard organization={organization} />
    </div>
  )
}

/** Separate from the main form on purpose: capturing/clearing a drawn
 * signature is its own self-contained "capture, then Save" interaction,
 * not a text field that belongs in the surrounding react-hook-form/zod
 * flow. Saves directly via useUpdateOrganization. */
function OrganizationSignatureCard({
  organization,
}: {
  ...
  function handleSave() {
    ...
    updateOrganization.mutate(
      { id: organization.id, input: { signature_image: dataUrl } },
      {
        onSuccess: () => {
  ...
  return (
    <div className="rounded-xl bg-card ring-1 ring-foreground/10">
      <div className="border-b px-4 py-3">
        <h2 className="text-sm font-semibold">{t("signatureTitle")}</h2>
        <p className="text-xs text-muted-foreground">
      ...
      <div className="flex flex-col gap-3 p-4">
        <SignaturePad
          key={organization.signature_image ?? "empty"}
          ref={padRef}
          initialImage={organization.signature_image}
          className="max-w-md"
        />
