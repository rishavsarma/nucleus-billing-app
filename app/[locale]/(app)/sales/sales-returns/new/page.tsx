...
export default function NewSalesReturnPage() {
  ...
  const invoiceOptions = [
    ...(selectedInvoice &&
    !returnableInvoices.some((inv) => inv.id === selectedInvoice.id)
  ...
  async function onSubmit(values: NewSalesReturnValues) {
    ...
    // the whole list.
    const invoice =
      selectedInvoice?.id === values.invoice_id
        ? selectedInvoice
        : await fetchInvoiceById(values.invoice_id)
    if (!invoice) return
      ...
      >
        <Field data-invalid={!!formState.errors.invoice_id}>
          ...
          <SearchableSelect
            ...
            onValueChange={(value) =>
              setValue("invoice_id", value, { shouldValidate: true })
            }
