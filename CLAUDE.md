...
`deliveries` and `delivery_persons` (the local-delivery feature) are the one deliberate exception to this financial-document treatment — they're operational tracking, not money, so both normal delete and free status transitions (including back to `pending`) are allowed there. Not yet built in this app (see "v3 schema features not yet wired up" below) — this is guidance for whenever that entity is added, not a description of existing code.
...
### v3 schema features not yet wired up in this app

The `db-schema/` files (`001`–`005`) already define these at the database level. Nothing in `app/`, `hooks/`, `lib/database/`, `components/`, `store/`, or `messages/*.json` references any of them yet (confirmed by grep across the whole tree — zero hits for `org_has_addon`, `subscription_status`, `business_type`, `organization_addon`, `delivery_person`, `deliveries`). Treat the notes below as **what to build, not what exists** — don't write code that assumes `Organization.subscription_status`, a `useAddons()` hook, or a `deliveries` API route already exist, and don't cite this section as evidence the app supports subscription gating, add-ons, business types, or delivery tracking today.

- **Base subscription gating.** Add `subscription_status` / `subscription_current_period_end` to the `Organization` interface in `lib/database/types.ts`; the existing `organizations` route/service/hooks already handle arbitrary column updates via `PUT`, so surfacing the field is mostly a types + UI change, gated the same way `is_active` already is (superadmin-only, trigger-enforced at the DB level regardless of what the app sends).
- **Business types.** `billing.business_types` (Retail, Pharmacy, Restaurant, Distributor/Wholesaler, Services) — set once at onboarding via `organizations.business_type_id`. Purely advisory/UX, never used for access control. Building it means a new `business_types` slice through all four CRUD layers (route/service/hook/types), plus a `business_type_id` field on `Organization`.
- **Add-on marketplace.** `billing.addons` (catalog) + `billing.organization_addon_subscriptions` (per-org entitlement, full history kept). Reads are `is_org_member(org_id)`-gated as usual; writes only happen through `billing.subscribe_org_to_addon(org_id, slug)` / `billing.cancel_org_addon(org_id, slug)`, both superadmin/service-role only — there's no payment gateway wired up yet, so any "buy an add-on" UI in the admin panel would call these via `.rpc()` as the superadmin, not a direct table insert. `billing.business_type_addon_recommendations` is the advisory-only join table for the onboarding screen ("as a pharmacy, you'll probably want X") — never read for enforcement.
- **Add-on-gated features, once any exist.** Combine `requireOrgId()`'s org resolution with a check against `billing.org_has_addon(org_id, 'slug')` (via `.rpc()`) before allowing the route to proceed — same shape as the DB-side RLS policies use, don't reimplement the entitlement check as a plain table read.
- **Public pricing calculator.** `business_types`, `addons`, and `business_type_addon_recommendations` are readable by **anonymous** requests as of the `005` patch (`for select using (true)`) — this is what would power a logged-out pricing calculator on a marketing site, once one exists. Write access to all three stays superadmin-only. Don't extend this "public read" treatment to any other table without an explicit ask.
- **Local delivery.** `billing.delivery_persons` / `billing.deliveries` — see the financial-documents section above for how they differ from invoices/bills (delete allowed, free status transitions). Also a full four-layer slice to add when built.
