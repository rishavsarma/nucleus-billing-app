...
**Check every FK in the body, not just the parent.** A `*_items` route needs a `verifyBelongsToOrg` call for the parent document (`invoice_id`/etc.) *and* one for every other FK the body carries. Which FKs those are differs per table — check the actual columns rather than assuming: `invoice_items` has `item_id` + `item_variant_id`, `purchase_bill_items` and `offer_items` have `item_id`, `sales_return_items` has `item_id` + `invoice_item_id` (via `verifyChildBelongsToOrg`, since `invoice_items` has no `org_id` of its own), `purchase_return_items` has `item_id` + `purchase_bill_item_id`. **`credit_note_items` and `debit_note_items` have no `item_id` column at all** — they're `description`/`amount` rows, so the parent check is the only one they need (an earlier version of this file claimed otherwise; the 2026-09-07 audit corrected it against the live schema).

These checks must be repeated on `PUT`, not just `POST` — the 2026-09-07 audit found `invoice_items` and `purchase_bill_items` re-verifying nothing (or only `item_variant_id`) on update, which let a line item be repointed at another org's item; RLS on a `*_items` table only validates the *parent document's* org, never the item's. Both are fixed.

The general rule: every foreign key accepted from the request body — not just the one used to resolve the org — needs its own `verifyBelongsToOrg` call on every write path, because RLS only validates the row's own `org_id`/immediate parent, never a sibling FK pointing at a third table. `payments.installment_id` was the costliest instance of missing this: it reaches a `security definer` trigger that settles the installment, so an unverified id was a cross-tenant write.
...
- **Attendance and field visits** (`db-schema/optimization/021_staff_attendance_and_visits.sql`) — `billing.staff_attendance` (one row per staff per day, unique on `staff_id + attendance_date`, status `present|absent|half_day|leave`) and `billing.staff_visits` (where a staff member went for work: `place`, `purpose`, `outcome`, optional `customer_id`, status `planned|completed|cancelled`). Both are **recorded by a logged-in member on the staff member's behalf** — this is a deliberate product decision (2026-09-14), consistent with staff being login-free. Don't add a self-check-in path without revisiting that. Pages: `catalog/staff/attendance` (daily sheet saved as one bulk upsert + monthly register) and `catalog/staff/visits`. Text-only by design: no photos/GPS, because the app has no file storage configured. Both tables carry a `guard_staff_record_refs()` trigger that rejects a `staff_id`/`customer_id` from another org, on top of the routes' own FK checks.
...
### Never pass the request body straight to the database

Route handlers must not do `.insert(body)` / `.update(body)`. Every write goes through `pickAllowed("<entity>", body)` from `lib/database/allowed-fields.ts`, which keeps only that entity's allow-listed columns.

This exists because the 2026-09-07 audit found the raw body reaching the database on 57 call sites, which made the security of every column depend on whether a trigger happened to guard it. Where one didn't, the client owned it — most seriously the invoice/bill totals (`total`, `amount_paid`, `subtotal`, `tax_total`, `discount_total`, `round_off_amount`), which any org member could set to anything with a plain `PUT`, since `recalc_invoice()` only ever runs from triggers on *other* tables and columns.

Four categories are excluded from every allow-list: identity/ownership (`id`, `org_id`, `created_by`, `created_at`, `updated_at`), trigger-derived money (document totals, `line_subtotal`/`line_tax`/`line_total`), trigger-assigned document numbers (`invoice_number` and friends — but *not* `vendor_invoice_number`, which is the vendor's own reference), and system state (`organizations.is_active`/`subscription_status`/`subscription_current_period_end`, `installments.payment_id`/`paid_at`). **Adding a column to a table does not expose it** — it has to be added to the allow-list on purpose. `lib/__tests__/allowed-fields.test.ts` asserts these exclusions hold across every entity, so a careless addition fails CI.

`db-schema/optimization/020_security_fixes.sql` enforces the same thing at the database level with `BEFORE UPDATE` guards that pin derived columns back to `OLD` unless `billing.internal_recalc` is set — so the invariant holds even if a route handler is wrong. If you add a new `recalc_*()` function, it **must** call `perform set_config('billing.internal_recalc', 'on', true)` before its `UPDATE`, or the guard will silently freeze that document's totals.

### Other API-layer invariants (2026-09-07 audit)

- **Never return a raw database error to the client.** Use `dbError(error, "entity:METHOD")` from `lib/api-response.ts`: it logs the real error server-side (Sentry picks it up) and returns a generic message plus a stable `code`. Raw Postgres text leaks table/column/constraint names and made schema enumeration practical. Same stable-`code` convention as the i18n section below.
- **Parse bodies with `readJson(request)`**, not `await request.json()` — the latter throws on malformed input and surfaces as a 500. `readJson` returns `null`; answer with `badRequest("invalid_json", ...)`.
- **User input never goes raw into a PostgREST filter string.** `applyListParams` wraps search terms in double quotes via `escapeFilterValue()`. Interpolating unescaped meant `,` `.` `()` were read as filter *syntax*, letting a caller inject conditions and enumerate columns through error messages. `pageSize` is clamped to `MAX_PAGE_SIZE` (100) there too.
- **Org settings writes are admin-level.** Use `requireAdminOf(orgId)`, which matches what `organizations_update` RLS already requires; `requireMemberOf` let a plain member through the handler only to get a confusing 404 back from RLS.
...
- `lib/redis.ts` (ioredis) and `lib/resend.ts` (Resend email) are thin client exports. (`app/api/redis/route.ts`, a leftover unauthenticated connectivity smoke test, was deleted in the 2026-09-07 audit — don't re-add a debug endpoint without an auth guard.)
