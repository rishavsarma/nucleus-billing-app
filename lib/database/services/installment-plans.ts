...
export async function fetchInstallmentPlanByInvoiceId(
  invoiceId: string
): Promise<InstallmentPlan | null> {
  const { data } = await api.get<InstallmentPlan | null>(
    "/database/installment_plans",
    {
...
export async function updateInstallmentPlan(
  ...
  const { data } = await api.put<InstallmentPlan>(
    "/database/installment_plans",
    input,
    { params: { id } }
  )
