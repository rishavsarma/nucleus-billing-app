...
export async function fetchSalesReturnById(id: string): Promise<SalesReturn> {
  const { data } = await api.get<SalesReturn>("/database/sales_returns", {
    params: { id },
  })
  return data
}
...
export async function fetchSalesReturnsPaginated(
  params: ListParams
): Promise<PaginatedResponse<SalesReturnWithRelations>> {
  const { data } = await api.get<PaginatedResponse<SalesReturnWithRelations>>(
    "/database/sales_returns",
    { params }
  )
  return data
}
...
export async function updateSalesReturn(
  ...
  const { data } = await api.put<SalesReturn>(
    "/database/sales_returns",
    input,
    { params: { id } }
  )
