...
export async function fetchPdfWatermarksAll(): Promise<PdfWatermark[]> {
  const { data } = await api.get<PdfWatermark[]>("/database/pdf_watermarks", {
...
export async function fetchPdfWatermarksPaginated(
  params: ListParams
): Promise<PaginatedResponse<PdfWatermark>> {
  const { data } = await api.get<PaginatedResponse<PdfWatermark>>(
    "/database/pdf_watermarks",
    { params }
  )
  return data
}

export async function createPdfWatermark(
  input: Partial<PdfWatermark>
): Promise<PdfWatermark> {
  const { data } = await api.post<PdfWatermark>(
    "/database/pdf_watermarks",
    input
  )
  return data
...
export async function updatePdfWatermark(
  id: string,
  input: Partial<PdfWatermark>
): Promise<PdfWatermark> {
  const { data } = await api.put<PdfWatermark>(
    "/database/pdf_watermarks",
    input,
    { params: { id } }
  )
