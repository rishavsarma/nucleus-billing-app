...
export async function fetchInstallmentsByPlanId(
  planId: string
): Promise<Installment[]> {
  const { data } = await api.get<Installment[]>("/database/installments", {
...
export async function fetchInstallmentsPaginated(
  params: ListParams
): Promise<PaginatedResponse<InstallmentWithInvoice>> {
  const { data } = await api.get<PaginatedResponse<InstallmentWithInvoice>>(
