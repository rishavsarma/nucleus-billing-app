...
export async function fetchTaxRatesPaginated(
  params: ListParams
): Promise<PaginatedResponse<TaxRate>> {
  const { data } = await api.get<PaginatedResponse<TaxRate>>(
    "/database/tax_rates",
    { params }
  )
  return data
...
export async function updateTaxRate(
  ...
  const { data } = await api.put<TaxRate>("/database/tax_rates", input, {
    params: { id },
  })
  return data
}
