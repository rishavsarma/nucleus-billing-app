...
export async function fetchOffersPaginated(
  params: ListParams
): Promise<PaginatedResponse<Offer>> {
  const { data } = await api.get<PaginatedResponse<Offer>>("/database/offers", {
    params,
  })
  return data
}
...
export async function updateOffer(
  ...
  const { data } = await api.put<Offer>("/database/offers", input, {
    params: { id },
  })
  return data
}
