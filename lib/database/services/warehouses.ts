...
export async function fetchWarehousesPaginated(
  params: ListParams
): Promise<PaginatedResponse<Warehouse>> {
  const { data } = await api.get<PaginatedResponse<Warehouse>>(
    "/database/warehouses",
    { params }
  )
  return data
...
export async function updateWarehouse(
  ...
  const { data } = await api.put<Warehouse>("/database/warehouses", input, {
    params: { id },
  })
  return data
}
