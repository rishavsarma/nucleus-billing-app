...
export async function fetchPurchasePaymentsByBillId(
  purchaseBillId: string
): Promise<PurchasePayment[]> {
  const { data } = await api.get<PurchasePayment[]>(
    "/database/purchase_payments",
...
export async function fetchPurchasePaymentsPaginated(
  params: ListParams
): Promise<PaginatedResponse<PurchasePaymentWithRelations>> {
  const { data } = await api.get<
    PaginatedResponse<PurchasePaymentWithRelations>
  >("/database/purchase_payments", { params })
  return data
}

/** `idempotencyKey`, when provided, is sent as the `Idempotency-Key` header
 * so a double-submit replays the original result instead of recording the
 * payment twice — see app/api/database/purchase_payments/route.ts and
 * lib/idempotency.ts. */
...
export async function updatePurchasePayment(
  ...
  const { data } = await api.put<PurchasePayment>(
    "/database/purchase_payments",
    input,
    {
      params: { id },
