...
export async function fetchPurchaseReturnItems(
  purchaseReturnId: string
): Promise<PurchaseReturnItem[]> {
  const { data } = await api.get<PurchaseReturnItem[]>(
    "/database/purchase_return_items",
    {
...
export async function createPurchaseReturnItem(
  ...
  const { data } = await api.post<PurchaseReturnItem>(
    "/database/purchase_return_items",
    input
  )
  return data
...
export async function updatePurchaseReturnItem(
  ...
  const { data } = await api.put<PurchaseReturnItem>(
    "/database/purchase_return_items",
    input,
    {
      params: { id },
