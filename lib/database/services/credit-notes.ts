...
export async function fetchCreditNotesPaginated(
  params: ListParams
): Promise<PaginatedResponse<CreditNoteWithCustomer>> {
  const { data } = await api.get<PaginatedResponse<CreditNoteWithCustomer>>(
    "/database/credit_notes",
    { params }
  )
  return data
}
...
export async function updateCreditNote(
  ...
  const { data } = await api.put<CreditNote>("/database/credit_notes", input, {
    params: { id },
  })
  return data
}
