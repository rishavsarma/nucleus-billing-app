...
export async function fetchSalesReturnItems(
  salesReturnId: string
): Promise<SalesReturnItem[]> {
  const { data } = await api.get<SalesReturnItem[]>(
    "/database/sales_return_items",
    {
...
export async function createSalesReturnItem(
  ...
  const { data } = await api.post<SalesReturnItem>(
    "/database/sales_return_items",
    input
  )
  return data
...
export async function updateSalesReturnItem(
  ...
  const { data } = await api.put<SalesReturnItem>(
    "/database/sales_return_items",
    input,
    {
      params: { id },
