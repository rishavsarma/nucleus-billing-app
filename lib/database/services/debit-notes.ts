...
export async function fetchDebitNotesPaginated(
  params: ListParams
): Promise<PaginatedResponse<DebitNoteWithVendor>> {
  const { data } = await api.get<PaginatedResponse<DebitNoteWithVendor>>(
    "/database/debit_notes",
    { params }
  )
  return data
}
...
export async function updateDebitNote(
  ...
  const { data } = await api.put<DebitNote>("/database/debit_notes", input, {
    params: { id },
  })
  return data
}
