...
export async function fetchCreditNoteItems(
  creditNoteId: string
): Promise<CreditNoteItem[]> {
  const { data } = await api.get<CreditNoteItem[]>(
    "/database/credit_note_items",
    {
...
export async function createCreditNoteItem(
  ...
  const { data } = await api.post<CreditNoteItem>(
    "/database/credit_note_items",
    input
  )
  return data
...
export async function updateCreditNoteItem(
  ...
  const { data } = await api.put<CreditNoteItem>(
    "/database/credit_note_items",
    input,
    {
      params: { id },
