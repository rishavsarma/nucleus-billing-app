...
export async function fetchDebitNoteItems(
  debitNoteId: string
): Promise<DebitNoteItem[]> {
  const { data } = await api.get<DebitNoteItem[]>(
    "/database/debit_note_items",
    {
...
export async function createDebitNoteItem(
  ...
  const { data } = await api.post<DebitNoteItem>(
    "/database/debit_note_items",
    input
  )
  return data
...
export async function updateDebitNoteItem(
  ...
  const { data } = await api.put<DebitNoteItem>(
    "/database/debit_note_items",
    input,
    {
      params: { id },
